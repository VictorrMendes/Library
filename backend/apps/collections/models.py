...
class Collection(models.Model):
    owner = models.ForeignKey(
        AUTH, on_delete=models.CASCADE, related_name="collections"
    )
    title = models.CharField(max_length=255)
    summary = models.TextField(blank=True)
    cover_image = models.ImageField(
        upload_to="collection_covers/", blank=True, null=True
    )
...
class ReadingList(models.Model):
    owner = models.ForeignKey(
        AUTH, on_delete=models.CASCADE, related_name="reading_lists"
    )
    title = models.CharField(max_length=255)
    summary = models.TextField(blank=True)
    cover_image = models.ImageField(
        upload_to="readinglist_covers/", blank=True, null=True
    )
    is_promoted = models.BooleanField(default=False)
    created_at = models.DateTimeField(auto_now_add=True)
    last_modified = models.DateTimeField(auto_now=True)
...
class ShelfStatus(models.TextChoices):
    ...
    READING = "reading", "Estou Lendo"
    COMPLETED = "completed", "Lido"
    LIKED = "liked", "Gostei"
    DROPPED = "dropped", "Abandonei"


class WantToRead(models.Model):
    user = models.ForeignKey(
        AUTH, on_delete=models.CASCADE, related_name="want_to_read"
    )
    ...
    def __str__(self):
        return (
            f"{self.user.username} → {self.series.name} [{self.status}]"
        )


class SmartFilter(models.Model):
    owner = models.ForeignKey(
        AUTH, on_delete=models.CASCADE, related_name="smart_filters"
    )
